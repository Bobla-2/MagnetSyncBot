...
class ManagerDB:
    ...
    def get_info_text_and_names(self, categories: list, search_title: str) -> list:
        """
        ...
        for db in self.__list_db:
            if any(cat == db[1] for cat in categories):
                name, url_str = db[0].get_names_and_url_title(search_title)
                out.append([db[1], name, url_str])
        return out
